const books = [
    ...
    booksToDisplay.forEach((book) => {
      ...
      article.innerHTML = `
              ...
              <div class="book-content">
                  <h2><a href="#" class="book-title">${book.title}</a></h2>
                  <img src="${book.imgSrc}" alt="${book.imgAlt}">
                  <p class="book-description">${book.description}</p>
              </div>
          `
  
      bookReviewsContainer.appendChild(article)
    })
  }
  ...
  function filterBooks() {
    ...
    const fiction = document.getElementById("fiction").value
  
    const filteredBooks = books.filter((book) => {
      return (
        (!readingLevel || book.ages === readingLevel) &&
